...
from (
            ...
            when (
                ...
                from (
                    ...
                    where itreg_repasse.cd_reg_repasse = 4
                        and nvl(trunc(itreg_repasse.dt_vigencia), trunc(itreg_amb.hr_lancamento)) <= trunc(itreg_amb.hr_lancamento)
                    ...
                    order by cd_convenio, cd_pro_fat asc
                ) v_itreg_repasse
                where rownum = 1
            ) = 100 then 'ANALISAR COM FATURAMENTO - COM PACOTE'
            when (
                -- Calculo do percentual a repassar para os medicos
                -- pegando da tabela de regras de repasse
                select vl_percent_pago
    ...
    where itreg_amb.cd_atendimento in (
        ...
        where s_itreg_amb.sn_pertence_pacote = 'S'
            and trunc(s_itreg_amb.hr_lancamento) between :P_DATAI and :P_DATAF
            and s_atendime.cd_prestador = :P_PRESTADOR
        )
        and prestador.tp_situacao = 'A' 
        and prestador.cd_conselho = 1
        and prestador.cd_tip_presta = 8
        and prestador.tp_vinculo in ('F')
        and itreg_amb.sn_pertence_pacote = 'N'
      --  and itreg_amb.cd_atendimento in (28395)
    ...
    select
        case
            when (
                ...
                where rownum = 1
            ) = 100 then 'ANALISAR COM FATURAMENTO - SEM PACOTE'
            when (
                -- Calculo do percentual a repassar para os medicos
                -- pegando da tabela de regras de repasse
                select vl_percent_pago
    ...
    join prestador on
        prestador.cd_prestador = nvl(itreg_amb.cd_prestador, :P_PRESTADOR)
    -- ATENCAO PARA O "NOT IN"
    --      ELE EXCLUI OS REGISTROS PERTENCENTES A PACOTES
    --      NA DATA SELECIONADA
    where itreg_amb.cd_atendimento not in (
        select distinct 
            s_itreg_amb.cd_atendimento 
            --, trunc(s_itreg_amb.hr_lancamento)
            --, s_itreg_amb.sn_pertence_pacote
            --, s_atendime.cd_prestador ATEND_PREST
            --, s_itreg_amb.cd_prestador ITREG_PREST
        from itreg_amb s_itreg_amb
        join atendime s_atendime on
            s_atendime.cd_atendimento = s_itreg_amb.cd_atendimento
        where s_itreg_amb.sn_pertence_pacote = 'S'
    ...
    and prestador.cd_tip_presta = 8
    and prestador.tp_vinculo in ('F')
    and trunc(itreg_amb.hr_lancamento) between :P_DATAI and :P_DATAF
